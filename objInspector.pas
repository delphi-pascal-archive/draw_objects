...
procedure TObjInspectForm.Initialize;
  ...
  procedure AddComponent(const key, val: string);
  begin
    with vle do
      with ItemProps[InsertRow(key, val, true)] do
      begin
      ...
      case typinfo.PropType(fObj, fPropList[i]) of
        tkClass:
          begin
            ...
            if GetObjectPropClass(fObj, fPropList[i]).InheritsFrom(TComponent) then
            begin
              if not assigned(PersistObj) then
                AddComponent(fPropList[i], '')
              else if TComponent(PersistObj).Name <> '' then
                AddComponent(fPropList[i], TComponent(PersistObj).Name)
              else
                AddComponent(fPropList[i], format('{%8.8x}',[cardinal(PersistObj)]));
            end
            else if PersistObj is TStrings then
              AddMultiLines(fPropList[i], RTrim(TStrings(PersistObj).Text))
      ...
      tkClass:
        begin
          ...
          begin
            ...
            vle.Cells[1,i] := RTrim(TStrings(PersistObj).Text)
          else if PersistObj is TFont then
...
begin
  ...
  with vle do
    ...
    begin
      ...
      if GetEnumProp(Obj, key) <> Value then
