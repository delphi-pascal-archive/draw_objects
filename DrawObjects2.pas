  ...
  begin
    ...
    try
      ...
      begin
        ...
        angle := GetAnglePt1FromPt2(flatPts[j],flatPts[j-1]);
        AngledCharOut(canvas, pt,
          s[i], angle, -charWidthDiv2, curveOffset, outline);
        inc(currentInsertionDist, charWidths[s[i]] -
          charWidthDiv2 + trunc(spcPxls) + round(frac(spcPxls*i)));
      end;

      //draw the curve (ie for debugging) ...
      //with flatPts[0] do moveto(x,y);
      //for i := 1 to ptCnt -1 do with flatPts[i] do lineto(x,y);
...
begin
  ...
  begin
    ...
    if (canvas.pen.Style <> psSolid) and (canvas.pen.Width > 1) then
      DrawDottedPolyline(Canvas, BtnPoints) else
      PolyLine(BtnPoints);
...
begin
  ...
  begin
    ...
    if (canvas.pen.Style <> psSolid) then Brush.Style := bsClear;
    if (canvas.pen.Style <> psSolid) and (canvas.pen.Width > 1) then
      DrawDottedPolyline(Canvas, [startPt, midPt, endPt]) else
      PolyLine([startPt, midPt, endPt]);
...
begin
  ...
  with Canvas do
  begin
    savedPenWidth := Pen.Width; //fBitmap canvas
    Pen.Width := 1;
    Brush.Style := bsClear;
    if SlopeLessThanOne(BtnPoints[0], BtnPoints[buttonCount-1]) then
    begin
      dx := 0;
      dy := savedPenWidth div 2;
      OffsetBtns(0,-dy);
      if odd(savedPenWidth) then inc(dy);
    end
    else begin
      dx  := savedPenWidth div 2;
      dy := 0;
      OffsetBtns(-dx,0);
      if odd(savedPenWidth) then inc(dx);
    end;
...
begin
  ...
  begin
    ...
    else if Filled and not Arrow1 and not Arrow2 then
    begin
...
begin
  ...
  //Anyhow, this PrintBitmap() function seems to solve these issues in TPic ...
...
begin
  ...
  angle := GetAnglePt1FromPt2(FromPt,ObjectMidpoint) + rotationAngle;
...
var
  ...
  begin
    ...
    angle1 := GetAnglePt1FromPt2(hPt, mp);
    angle2 := GetAnglePt1FromPt2(wPt, mp);
    result :=
...
begin
  ...
  begin
    ...
    angle_offset := GetAnglePt1FromPt2(NewPt,mp) - angle_increment*StartPtIdx;
    if odd(StartPtIdx) then
      i := 1 else
      i := 0;
