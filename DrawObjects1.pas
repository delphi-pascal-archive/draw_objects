...
Copyright:       © 2006-2007 Angus Johnson
********************************************************************************
...
Changes:
  ...
  5 Dec 06       * Changed BeginRotate() & EndRotate() to BeginTransform() and
                 EndTransform() since these methods are required for both Zoom()
                 and Rotate(). These methods preserve proportional scaling and
                 orientation relative to a starting state when multiple
                 Rotate() or Zoom() calls are made (eg when moving a slider).
...
function GetAnglePt1FromPt2(pt1, pt2: TPoint): single;
...
begin
  ...
  //nb: not fully tested but should be OK considering margins ...
...
begin
  ...
  begin
    ...
    begin
      ...
      if (fPressedBtnIdx < 0) and fCanMove then
      begin
        fMovingPt := Point(X,Y);
        fMoving := true;
      end;
...
begin
  ...
  begin
    ...
    end else if fMoving then //(ie Moving, not resizing)
    begin
...
begin
  ...
  if (csDesigning in ComponentState) or Focused then
    for i := ButtonCount-1 downto 0 do //nb: get higher indexed btn b4 lower one
...
var
  ...
  begin
    ...
    propVal  := trim(copy(str,i+1, length(str)));
  end;
...
begin
  ...
  angleToFromPt := GetAnglePt1FromPt2(FromPt,mp);
  angleToTopRtCnr := GetAnglePt1FromPt2(tr,mp);
...
procedure DrawDottedPolyline(Canvas: TCanvas; pts: array of TPoint);
var
  i, dx, dy, savedPenWidth: integer;

  procedure OffsetPts(x,y: integer);
  var
    i: integer;
  begin
...
begin
  //Windows unfortunately draws all lines with width > 1 as solid lines, so...
  with Canvas do
  begin
    savedPenWidth := Pen.Width;
    Pen.Width := 1;
    Brush.Style := bsClear;
    if SlopeLessThanOne(pts[0], pts[high(pts)]) then
    begin
      dx := 0;
      dy := savedPenWidth div 2;
      OffsetPts(0,-dy);
    end
    else begin
      dx  := savedPenWidth div 2;
      OffsetPts(-dx,0);
    end;
    //OK, draw the line here ...
    for i := 1 to savedPenWidth do
    begin
      PolyLine(pts);
      if dx = 0 then
        OffsetPts(0,1) else
...
begin
  ...
  UpdateConnectionPoints(nil); //ammended 6-dec-2005
end;
//------------------------------------------------------------------------------
...
begin
  ...
  //? adjust orientation
  if not fAutoOrientation then //
  else if HorzSlope and (fOrientation = oVertical) then Orientation := oHorizontal
  else if not HorzSlope and (fOrientation = oHorizontal) then
...
begin
  if Moving and (assigned(fConnection1) or assigned(fConnection2)) then exit;
  inherited;
  if (fPressedBtnIdx < 0) then
    exit
